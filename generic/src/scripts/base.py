...
class DebianSetup:
    ...
    def run(self):
        ...
        try:
            ...
            ret_code, output, _ = self.run_command("grep -q container=lxc /proc/1/environ && echo \"Running in LXC\" || echo \"Not running in LXC\"", shell=True, check=False)
            in_lxc = ret_code == 0 and "Running in LXC" in output
            
            if in_lxc:
                logger.info(f"{blue('Detected LXC container environment, using root for all operations')}", {'color': True})
                
                # For LXC, we always want to run as root
                if os.geteuid() != 0:
                    logger.info(f"{yellow('Not running as root in LXC. Restarting with root privileges...')}", {'color': True})
                    script_path = os.path.abspath(__file__)
                    # Use sudo to restart as root
                    os.system(f"sudo -E python3 {script_path}")
                    sys.exit(0)
            elif os.geteuid() != 0:
                # Not in LXC and not running as root
...
""")
    ...
    def detect_non_root_user(self) -> str:
        ...
        if in_lxc:
            # When in LXC, always use root
            logger.info("Detected LXC container environment, using root user for all operations")
            return "root"
        if os.geteuid() == 0:
...
'''
                ...
                # First write debug log entry
                with open("/home/user/debug.log", "a") as debug_log:
                    debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: Starting NVM installation for root user\n")
                
                # List the root directory for debugging
                self.run_command("ls -la /root >> /home/user/debug.log 2>&1", shell=True)
                ...
                with open("/home/user/debug.log", "a") as debug_log:
                    debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: Running NVM install command for root with explicit HOME\n")
                
                # Run the NVM installer with HOME explicitly set to /root
                nvm_install_cmd_root = "export HOME=/root && curl -v -o- https://raw.githubusercontent.com/nvm-sh/nvm/master/install.sh | bash"
                ret_code, stdout, stderr = self.run_command(nvm_install_cmd_root + " >> /home/user/debug.log 2>&1", shell=True, check=False)
                
                # Log the result of the NVM installation
                with open("/home/user/debug.log", "a") as debug_log:
                    debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: NVM install command returned code {ret_code}\n")
                    if stdout:
                        debug_log.write(f"NVM stdout: {stdout}\n")
                    if stderr:
                        debug_log.write(f"NVM stderr: {stderr}\n")
                ...
                with open("/home/user/debug.log", "a") as debug_log:
                    if nvm_check:
                        debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: NVM directory exists in /root/.nvm: {nvm_check}\n")
                    else:
                        debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: NVM directory NOT found in /root/.nvm\n")
                        ...
                        debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: Found .nvm directories: {alt_nvm_check}\n")
                        
                        # If NVM was installed in /home/user/.nvm instead, copy it to /root/.nvm
                        if os.path.exists("/home/user/.nvm") and not os.path.exists("/root/.nvm"):
                            debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: Copying NVM from /home/user/.nvm to /root/.nvm\n")
                            self.run_command("cp -r /home/user/.nvm /root/.nvm", shell=True)
...
''')
                        ...
                        with open("/home/user/debug.log", "a") as debug_log:
                            debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: Sourcing updated .bashrc returned code {ret_code}\n")
                    else:
                        logger.info("NVM source commands already exist in root's .bashrc")
                        with open("/home/user/debug.log", "a") as debug_log:
                            debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: NVM source commands already exist in root's .bashrc\n")
                else:
...
''')
                    with open("/home/user/debug.log", "a") as debug_log:
                        debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: Created new .bashrc for root with NVM configuration\n")
                
                # Try to install Node.js using NVM for root with explicit HOME
                with open("/home/user/debug.log", "a") as debug_log:
                    debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: Attempting to install Node.js with NVM for root\n")
                ...
                ret_code, stdout, stderr = self.run_command(nvm_install_node_cmd_root + " >> /home/user/debug.log 2>&1", shell=True, check=False)
                
                with open("/home/user/debug.log", "a") as debug_log:
                    debug_log.write(f"{datetime.datetime.now().strftime('%a %b %d %H:%M:%S %Z %Y')}: Node.js install returned code {ret_code}\n")
                    if stdout:
                        debug_log.write(f"Node.js stdout: {stdout}\n")
                    if stderr:
                        debug_log.write(f"Node.js stderr: {stderr}\n")
    ...
    def setup_user(self):
        ...
        # In LXC, if we're using root as the primary user, skip user setup
        if in_lxc and self.current_non_root_user == "root":
            logger.info("Running in LXC container as root, skipping user setup")
            return
    ...
    def configure_ssh(self):
        ...
        # Check if running in LXC container
        logger.info("Checking if running in LXC container...")
        ret_code, output, error = self.run_command("grep -q container=lxc /proc/1/environ && echo \"Running in LXC\" || echo \"Not running in LXC\"", shell=True, check=False)
        
        in_lxc = False
        if ret_code == 0 and "Running in LXC" in output:
            logger.info("Detected system is running in LXC container")
            in_lxc = True
        elif ret_code == 0 and "Not running in LXC" in output:
            logger.info("System is not running in LXC container")
        else:
            logger.warning(f"Could not determine if running in LXC: {error}. Defaulting to standard configuration.")
            
        # Write new SSH configuration
        logger.info("Writing new SSH configuration...")
        if in_lxc:
            # For LXC containers, allow root login and don't restrict users
...
""")
            ...
            # For non-LXC, standard configuration with restricted root login and allowed users
            with open("/etc/ssh/sshd_config", "w") as f:
...
ClientAliveInterval 300
ClientAliveCountMax 2
AllowUsers {self.current_non_root_user}
""")
    ...
    def discover_smb_shares(self):
        ...
        _, interface_output, _ = self.run_command("ip route get 8.8.8.8 | grep -oP '(?<=dev\\s)\\w+' | head -1", shell=True)
        interface = interface_output.strip()

        _, ip_addr_output, _ = self.run_command(f"ip -4 addr show {interface} | grep -oP '(?<=inet\\s)\\d+(\.\\d+){{3}}'", shell=True)
        ip_addr = ip_addr_output.strip()

        _, netmask_output, _ = self.run_command(f"ip -4 addr show {interface} | grep -oP '(?<=inet\\s)\\d+(\.\\d+){{3}}/\\d+' | cut -d '/' -f 2", shell=True)
        netmask = netmask_output.strip()

        _, subnet_output, _ = self.run_command(f"ipcalc -n {ip_addr}/{netmask} | grep Network | awk '{{print $2}}'", shell=True)
...
""")
    ...
    def setup_ssh_keys(self):
        ...
        if not os.path.exists(ssh_dir):
            ...
            # Only change ownership if not in LXC as root
            if not (in_lxc and self.current_non_root_user == "root"):
        ...
        if not os.path.exists(os.path.join(ssh_dir, "id_rsa")) and not os.path.exists(os.path.join(ssh_dir, "id_rsa.pub")):
            ...
            # In LXC as root, generate the key directly
            if in_lxc and self.current_non_root_user == "root":
                self.run_command(f"ssh-keygen -t rsa -N \"\" -f {os.path.join(ssh_dir, 'id_rsa')}", shell=True)
            else:
    ...
    def finalize_script(self):
        ...
        if self.error_flag:
            ...
            # Check if we're in LXC container to display correct SSH configuration
            ret_code, output, _ = self.run_command("grep -q container=lxc /proc/1/environ && echo \"Running in LXC\" || echo \"Not running in LXC\"", shell=True, check=False)
            in_lxc = ret_code == 0 and "Running in LXC" in output
            if in_lxc:
                logger.info("- System detected as running in LXC container")
                logger.info("- Using root user for all operations")
                logger.info("- Root login is enabled")
                logger.info("- No user restrictions are in place")
            else:
            ...
            # Get user's home directory for final message
            _, user_home, _ = self.run_command(f"getent passwd {self.current_non_root_user} | cut -d: -f6", shell=True)
            ssh_dir = os.path.join(user_home, ".ssh")

            logger.info(f"Your SSH key has been generated and is located at: {cyan(os.path.join(ssh_dir, 'id_rsa'))}", {'color': True})
            logger.info(f"Public key: {cyan(os.path.join(ssh_dir, 'id_rsa.pub'))}", {'color': True})
            logger.info(f"Your authorized_keys file is set up at: {cyan(os.path.join(ssh_dir, 'authorized_keys'))}", {'color': True})
            logger.info("")
            
            # Adjust connection instructions based on LXC vs non-LXC
            if in_lxc:
                logger.info(f"You can now connect to this container using: {green('ssh root@container_ip')}", {'color': True})
            else:
